...
public class TomcatJDBCDAO {
    ...
    public static boolean isUseReadWaiterSeparate = false;
    /**
    ...
    private static Tuple2<String, String> sqlConvertNative(String sql, int table_slice) {
        ...
        if (tableList.size() == 0)
            throw new JDBCException("sql: '" + sql + "' ,no matching table name, please use: {{?TABLE_NAME}}");
    ...
    private static Tuple2<JDBCSessionFacadeWrap, List<String>> getDaoOp(DataBaseType dataBaseType,boolean isCallProcedure,int type, List<String> sqlList, int db_slice, int table_slice) {
        // 返回: 库名/ 获取原始sql列表
        ...
        //2. 获取连接池操作对象
        JDBCSessionFacade op = TomcatJDBC.getFacade(dataBaseType,tuple.getValue0(),true);

        JDBCSessionFacadeWrap wrap = isUseReadWaiterSeparate && type == 1 ? new ReadWriteSeparateJDBCFacadeWrap(op) : new MasterSlaveSyncJDBCFacadeWrap(op);
    ...
    private static Tuple2<JDBCSessionFacadeWrap, List<String>> getQueryDaoOpMany(DataBaseType dataBaseType,String sql, int... table_slices) {
        ...
        //2. 获取连接池操作对象
        JDBCSessionFacade op = TomcatJDBC.getFacade(dataBaseType,database,true);

        return new Tuple2<>(new ReadWriteSeparateJDBCFacadeWrap(op), sqlList);
    }
    ...
    private static Tuple2<JDBCSessionFacadeWrap, String> getNativeSql(DataBaseType dataBaseType,String sql, int[] table_slices) {
        ...
        //2. 获取连接池操作对象
        JDBCSessionFacade op = TomcatJDBC.getFacade(dataBaseType,database,true);
        return new Tuple2<>(new ReadWriteSeparateJDBCFacadeWrap(op), sql);
    }
    ...
    public static List<Object[]> call(String sql, Object[] inParam, int outParamStartPos, int[] outParamTypes) {
        List<String> sqlList = new ArrayList<>();
        sqlList.add(sql);
        Tuple2<JDBCSessionFacadeWrap, List<String>> tuple = getDaoOp(DataBaseType.mysql,true,0, sqlList,0,0);
    ...
    public static int update(DataBaseType dataBaseType,String sql, Object[] params, int db_slice, int table_slice) {
        List<String> sqlList = new ArrayList<>();
        sqlList.add(sql);
        Tuple2<JDBCSessionFacadeWrap, List<String>> tuple = getDaoOp(dataBaseType,false,0, sqlList, db_slice, table_slice);
    ...
    public static List<Object[]> queryByOriginal(DataBaseType dataBaseType,String databaseName, String sql, Page page, Object... params) {
        JDBCSessionFacade facade = TomcatJDBC.getFacade(dataBaseType,databaseName,true);
        if (facade == null) throw new IllegalArgumentException("数据库类型或库名不正确,找不到可执行的数据库对象");
    ...
    public static List<Object[]> query(DataBaseType dataBaseType,String sql, Object[] params, Page page, int db_slice, int table_slice) {
        List<String> sqlList = new ArrayList<>();
        sqlList.add(sql);
        Tuple2<JDBCSessionFacadeWrap, List<String>> tuple = getDaoOp(dataBaseType,false,1, sqlList, db_slice, table_slice);
