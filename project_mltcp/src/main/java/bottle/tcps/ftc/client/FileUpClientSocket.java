...
public class FileUpClientSocket extends FtcTcpActionsAdapter{
    ...
    public void receiveString(Session session, String message) {
        ...
        if (message.equals("start")){
            try {
                Log4j.info("[客户端] 连接建立成功 >> "+ session.getSocket().getLocalAddress() +" <---> " + session.getSocket().getRemoteAddress());
            } catch (IOException e) {
                e.printStackTrace();
            }
            return;
...
//
    ...
    private void transferStream(Map<String, String> map) {
        try{
            Log4j.info(flag + " 开始传输 "+curFile.getFullPath());
    ...
    private void transOver(boolean isCloseConn) {
        try {
            //Log4j.info(flag+" ( "+ curFile +" )结束传输 是否正在使用中: "+ isUsing + "  , 连接是否关闭: "+ isCloseConn);
            //判断连接中断
            if (isCloseConn){
                ...
                if (isUsing){
                    ...
                    if (fbcThreadBySocketList!=null && fbcThreadBySocketList.ftcBackupClient!=null){
                        Log4j.info("连接中断,尝试加入任务(" +curFile+ ") 到队列" );
                        if (curFile!=null){
                            try {
