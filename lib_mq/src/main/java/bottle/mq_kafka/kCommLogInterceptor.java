...
//        if(isPrint) Log4j.debug("ProducerInterceptor onAcknowledgement:\t主题:" + metadata.topic()+" ,分区:"+ metadata.partition()+" ,偏移量:"+ metadata.offset());
        if (exception != null) {
            if(isPrint) Log4j.error("ProducerInterceptor onAcknowledgement exception", exception);
        }
    }

    @Override
    public ConsumerRecords<String, String> onConsume(ConsumerRecords<String, String> records) {
        if(isPrint) Log4j.debug("可消费消息条数\t" + records.count());
        return records;
    }

    @Override
    public void onCommit(Map<TopicPartition, OffsetAndMetadata> offsets) {
        if(isPrint) Log4j.debug("提交消息偏移\n\t" + offsets);
    }
